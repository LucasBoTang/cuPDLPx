...
#define ALLOC_AND_COPY(dest, src, bytes)  \
    ...
    ALLOC_AND_COPY(state->constraint_matrix->row_ptr, working_problem->constraint_matrix_row_pointers, (n_cons + 1) * sizeof(int));
...
__global__ void halpern_update_kernel(const double *initial_primal, double *current_primal,
                                      const double *reflected_primal,
                            ...
                            const pdhg_parameters_t *params)
{
    ...
    {   
        if (params->verbose) {
            printf("Applying diagonal scaling after inner iteration %d\n",
                   state->inner_count);
        }
